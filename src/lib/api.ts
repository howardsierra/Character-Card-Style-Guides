...
export async function extractSlotsFromTemplate(
  ...
  const prompt = `Analyze the following Character Card Template and extract EVERY SINGLE individual field, property, or slot that a user should fill out to create a character card using this template.
...
export async function extractSlotsFromGuide(
  ...
  // Fallback to AI if regex found nothing
  const prompt = `Analyze the following Style Guide and extract EVERY SINGLE individual field, property, or slot that a user should fill out to create a character card using this guide.
Return ONLY a JSON array of objects, where each object has a "name" (the exact name of the field/property from the guide, e.g., "Background", "Scenario", "Appearance") and a "description" (a brief explanation of what should go in this field).
Do not group fields together. If the guide has a section called "Appearance" with sub-fields like "Height", "Weight", and "Eye Color", you MUST extract "Height", "Weight", and "Eye Color" as separate fields.
Do not include fields that are generated by the AI (like First Message, Example Messages, etc.).
Do NOT include "Name" or "Core Concept/Archetype" as these are already provided by default.

...
// Be careful with // comments, they might be in URLs like http://
// Let's just remove lines that start with // or spaces then //
...
while ((match = regex.exec(cleanContent)) !== null) {
  ...
  if (fullTag.trim().endsWith('/>')) continue;
  
  // Ignore known self-closing tags that might not have />
  if (['img', 'input', 'br', 'hr', 'path', 'circle', 'line', 'rect', 'Textarea', 'Input'].includes(tagName)) continue;
